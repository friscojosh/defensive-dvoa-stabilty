...
dvoa <- read_csv("data/defensive_dvoa_2009-18.csv")
...
total_dvoa_stability <- glance(total_dvoa_model) %>%
   mutate(metric = "Total Defensive DVOA",
...
ggplot(data = dvoa_joined, aes(x = total_dvoa.x, y = total_dvoa.y)) +
   ...
   geom_smooth() +
   theme_bw() +
   labs(x = "Year Y Total Defensive DVOA",
        y = "Year Y + 1 Total Defensive DVOA",
        title = "Total Defensive DVOA is Unstable Year-to-Year",
...
pass_dvoa_stability <- glance(pass_dvoa_model) %>%
   mutate(metric = "Defensive Passing DVOA",
          r.squared = round(r.squared, 3)) %>%
   select(metric, r.squared)
...
rush_dvoa_stability <- glance(rush_dvoa_model) %>%
   mutate(metric = "Defensive Rushing DVOA",
          r.squared = round(r.squared, 3)) %>%
   select(metric, r.squared)
